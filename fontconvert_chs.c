...
const uint16_t ucodes[] = {
//日一二三四五六星期
0x65e5, 0x4e00, 0x4e8c, 0x4e09, 0x56db, 0x4e94, 0x516d, 0x661f, 0x671f
};

int main(int argc, char *argv[]) {
	int                i, j, err, size, first=0, last=sizeof(ucodes)/sizeof(uint16_t)-1,
	...
	for(i=first, j=0; i<=last; i++, j++) {
		...
		if((err = FT_Load_Char(face, ucodes[i], FT_LOAD_TARGET_MONO))) {
